...
declare_id!("PjG6i92qPk5hpFhmNBd1RWuPt3keH9xBSqn46dc4b5w");
...
pub mod prediction_market {
    ...
    if bet_yes {
        position.yes_amount = position.yes_amount.checked_add(amount).ok_or(MarketError::Overflow)?;
    }
    else{
        position.no_amount = position.no_amount.checked_add(amount ).ok_or(MarketError::Overflow)?;
...
pub fn resolve_market(ctx: Context<ResolveMarket>, outcome: bool) -> Result<()> {
    ...
    pub fn claim_winning(ctx: Context<ClaimWinning>) -> Result<()>{
        ...
        .checked_div(total_losing_pool as u128)
